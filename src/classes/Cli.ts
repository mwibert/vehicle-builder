...
class Cli {
  ...
  printWheelie(vehicle: Car | Truck | Motorbike) {
    if ("wheelie" in vehicle) {
    ...
    inquirer
      ...
      .then((answers) => {
        ...
        if (answers.vehicleToTow.vin === truck.vin) {
          console.log(
            "This vehicle cannot tow itself, please select another action"
          );
        } else {
    ...
    inquirer
      ...
      .then((answers) => {
        ...
        if (answers.action === "Print details") {
          ...
          // find the selected vehicle and tow it if it is a Truck
          for (let i = 0; i < this.vehicles.length; i++) {
            if (this.vehicles[i].vin === this.selectedVehicleVin) {
              if (this.vehicles[i] instanceof Truck) {
                this.findVehicleToTow(this.vehicles[i] as Truck);
                return; // return to prevent calling performActions immediately
              } else {
